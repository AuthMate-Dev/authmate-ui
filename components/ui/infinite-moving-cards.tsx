  ...
  return (
    ...
    >
      ...
      >
        {items.map((item, idx) => (
          <motion.li
            className="w-[350px] max-w-full relative rounded-2xl border border-b-0 flex-shrink-0 border-slate-700 px-8 py-6 md:w-[450px] card-gradient"
            key={item.name}
              ...
              <div className="relative z-20 mt-6 flex flex-row items-center">
                {item.avatar && (
                  <span className="flex flex-col gap-1">
                    <img src={item.avatar || "/placeholder.svg"} alt={item.name} className="w-10 h-10 rounded-full" />
                  </span>
                )}

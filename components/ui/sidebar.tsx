...
const Sidebar = ({ children, className, isOpen = true, onToggle, width = "16rem" }: SidebarProps) => {
  ...
  return (
    <motion.aside
      ...
      className={cn("relative flex flex-col bg-background border-r border-border h-full overflow-hidden", className)}
    >
      {onToggle && (
        <Button
          variant="ghost"
...
const SidebarContent = ({ children, className }: { children: React.ReactNode; className?: string }) => (
  <div className={cn("flex-1 p-2 space-y-1", className)}>{children}</div>
)

const SidebarItem = ({ icon: Icon, label, href, onClick, isActive, className }: SidebarItemProps) => {
  const content = (
    <motion.div
